...
function ArrowOverlay({ arrows, prefix }) {
  ...
  const markerId = `${prefix}-arrowhead`;
    ...
    <svg className="arrow-overlay" viewBox={`0 0 ${GRID_PIXEL_SIZE} ${GRID_PIXEL_SIZE}`} preserveAspectRatio="none" aria-hidden="true">
      <defs>
        <marker id={markerId} viewBox="0 0 8 8" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">
          <path d="M0,0 L8,4 L0,8 z" className="arrow-head-shape" />
        </marker>
      </defs>
      {arrows.map((arrow, idx) => {
        ...
        return (
          <g key={`${prefix}-arrow-${idx}`}>
            <path d={d} className="arrow-line" markerEnd={`url(#${markerId})`} />
      ...
      {result && (
        <section className="panel result">
          ...
          {typeof result.solutionCount === 'number' && (
            <p>
              <strong>Classic Solution Count:</strong>{' '}
          ...
          {category === 'classic' && result.solved && (
            <button
              ...
              disabled={loading || classicAnalysisLoading}
            >
